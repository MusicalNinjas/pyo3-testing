...
/// 
/// A second macro with the same name followed by an underscore (`_`) will also be created which
/// provides the initial `Result`, it can be called using `python_function_!()`. This is primarily
/// useful for validating that Errors are raised. Otherwise it must be `unwrap()`-ed to provide
/// a `Bound<'py, PyAny>` and then `.extract().unwrap()`-ed to get to the underlying value.
///
/// ### Note:
...
/// 1. The code inside the block must be valid rust which returns a `PyResult<T>`. That means you
/// should use the relevant `py_function_!()` (underscore) macro in a `with_py_raises` block.
